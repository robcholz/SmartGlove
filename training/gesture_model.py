...
IMU_FEATURE_INDICES = [FEATURE_COLUMNS.index(name) for name in ["acc_x", "acc_y", "acc_z", "vec_x", "vec_y", "vec_z"]]
LABEL_COLUMN = "label"
...
class GestureMLP(nn.Module):
    def __init__(
        ...
        super().__init__()
        input_dim = sequence_length * feature_count
...
def preprocess_sequence_features(features: np.ndarray) -> np.ndarray:
    processed = features.astype(np.float32, copy=True)
    imu_values = processed[:, :, IMU_FEATURE_INDICES]
    imu_means = imu_values.mean(axis=1, keepdims=True)
    processed[:, :, IMU_FEATURE_INDICES] = imu_values - imu_means
    return processed


def fit_feature_scaler(features: np.ndarray) -> StandardScaler:
    features = preprocess_sequence_features(features)
...
def export_onnx(model: nn.Module, sample_shape: tuple[int, int], output_path: Path) -> None:
    sequence_length, feature_count = sample_shape
...
def build_scaler_from_metadata(metadata: ModelMetadata) -> StandardScaler:
    ...
    scaler.n_features_in_ = len(metadata.feature_columns)
    return scaler
...
def write_rust_metadata(
    ...
    espdl_input_shape = metadata.espdl_input_shape or [1, metadata.sequence_length, len(metadata.feature_columns)]
...
pub const MODEL_FEATURE_COUNT: usize = {len(metadata.feature_columns)};
